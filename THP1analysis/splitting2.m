...
%same code as splitting1, but uses cells from second half of the data in
%the small subsets to get a different cell population
validation=[];
test6=[];
test12=[];
test23=[];
...
for i=1:35
    ...
    pcontrol=[pcontrol;p];
    for j=floor(numberofcells/2)+1:floor(numberofcells/2)+6
        test6=[test6;thesecells(j,:)];
        zombie6=[zombie6;Z];
    end
    effects6=[effects6;log(mean(thesecells(1:6,:))./meancontrols)];
    for j=floor(numberofcells/2)+1:floor(numberofcells/2)+12
        test12=[test12;thesecells(j,:)];
        zombie12=[zombie12;Z];
    end
    effects12=[effects12;log(mean(thesecells(1:12,:))./meancontrols)];
    for j=floor(numberofcells/2)+1:floor(numberofcells/2)+23
        test23=[test23;thesecells(j,:)];
        zombie23=[zombie23;Z];
    end
     for j=floor(numberofcells/2)+1:min(numberofcells,floor(numberofcells/2)+40)
        test40=[test40;thesecells(j,:)];
        zombie40=[zombie40;Z];
     end
    for j=floor(numberofcells/2)+1:min(numberofcells,floor(numberofcells/2)+50)
        test50=[test50;thesecells(j,:)];
        zombie50=[zombie50;Z];
    end
    effects23=[effects23;log(mean(thesecells(1:23,:))./meancontrols)];
    [h,p]=ttest2(thesecells(numberofcells/2+1:numberofcells/2+23,:),controlcells(:,9:end));
    p23=[p23;p];
    [h,p]=ttest2(thesecells(numberofcells/2+1:numberofcells/2+12,:),controlcells(:,9:end));
    p12=[p12;p];
    [h,p]=ttest2(thesecells(numberofcells/2+1:numberofcells/2+6,:),controlcells(:,9:end));
    p6=[p6;p];
    if density(i)>79
    effects40=[effects40;log(mean(thesecells(floor(numberofcells/2)+1:floor(numberofcells/2)+39,:))./meancontrols)];
    else
         effects40=[effects40;zeros(1,130)];
    end
    if density(i)>100
    effects50=[effects50;log(mean(thesecells(floor(numberofcells/2)+1:floor(numberofcells/2)+49,:))./meancontrols)];
    else
        effects50=[effects50;zeros(1,130)];
    end
    [h,p]=ttest2(thesecells(floor(numberofcells/2)+1:floor(min(numberofcells/2+49,numberofcells)),:),controlcells(:,9:end));
    p50=[p50;p];
    [h,p]=ttest2(thesecells(floor(numberofcells/2)+1:floor(min(numberofcells/2+39,numberofcells)),:),controlcells(:,9:end));
    p40=[p40;p];
...
writematrix(validation,fullfile(mypath,'validation.csv'))
writematrix(test6,fullfile(mypath,'test6.csv'))
writematrix(test12,fullfile(mypath,'test12.csv'))
writematrix(test23,fullfile(mypath,'test23.csv'))
writematrix(test40,fullfile(mypath,'test40.csv'))
writematrix(test50,fullfile(mypath,'test50.csv'))
...
figure,
for i=1:35
     if (sum(pcontrol(i,:)<0.05)>0)&(sum(abs(effects50(i,:))>0)>0)%&sum(effects50(i,:))~=0
        scatter(effectsvalidations(i,(pcontrol(i,:)<0.05)),effects50(i,(pcontrol(i,:)<0.05)),'.')
        hold on
    end
end
title('naive effects, 50 cells split2')
