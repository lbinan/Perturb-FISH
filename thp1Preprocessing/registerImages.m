function registeredImage=registerImages(imageref,thisimage)
fixed=imageref(:,:,28);%try 28 and 29 to find best results, finding bead image
data=fixed;
...
bw4=bw3;
%%
moving=thisimage(:,:,22);%and this one probably should stay 22 (this is to find bead image in subsequent rounds)
data=moving;
h=histFcn(data);
ccum=zeros(1,h.NumBins);
for i=1:h.NumBins
...
if max(abs(shift))>200 | sum(sum(bw4))<2500 | sum(sum(bw3))<2500 %if calculated displacement is very large, something went wrong so do again with different thresholds
...
if max(abs(shift))>200 | sum(sum(bw4))<2500 | sum(sum(bw3))<2500 %again redoing failed translations
