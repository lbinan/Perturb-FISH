...
%         thisImage=double(thisImage(:,:,24))/7500;
    elseif thisFOV<100
        thispath=(fullfile(mypath,strcat('stack_0_00',num2str(thisFOV),'.inf')));
...
%         thisImage=double(thisImage(:,:,24))/7500;
    ...
    for i=10:31
    temp=imbinarize(thisImage(:,:,i),'adaptive');
    bw(:,:,i-9)=(temp-bwareaopen(temp,20000))>0;
end
mask=bwareaopen(max(bw(:,:,:),[],3),500);
for i=10:31
    temp=imbinarize(thisImage(:,:,i),'adaptive');
end
empty=imcomplement(imdilate(mask,strel('disk',10)));
nuclei=mask|(empty&max(temp(:,:,:),[],3));
    nuclei=imdilate(nuclei,strel('disk',2));
    split=bwareaopen(nuclei,10000);
    keep=(nuclei-split)>0;
    D = bwdist(~split);
    D = -D;
    I3 = imhmin(D,2);
    ...
    mymask=((resplitnuclei>0)+(keep>0))>0;
    figure, imshow(mymask)
       splitagain=bwareaopen(bwpropfilt(mymask,'Solidity',[0 0.95]),10000);
    keep=(mymask-splitagain)>0;
    D = bwdist(~splitagain);
    D = -D;
    I3 = imhmin(D,0.55);
    resplitnuclei = watershed(I3);
    resplitnuclei(~splitagain)=0;
    mymask=bwareaopen(((resplitnuclei>0)+(keep>0))>0,2000);
    dapi(contour)=0;
    dapi=flipdim(mymask,2);
    dapi=imrotate(dapi,-91);
    imwrite(dapi,fullfile(nucleipath,strcat('nuclei',num2str(thisFOV),'.tif')));
...
%     X=imdilate(X,strel('disk',25));
    seedsmosaic(floor((thisY-1)*1843.2)+1:floor((thisY-1)*1843.2)+size(rotatedtemplate,1),floor((thisX-1)*1843.2)+1:floor((thisX-1)*1843.2)+size(rotatedtemplate,1))=max(seedsmosaic(floor((thisY-1)*1843.2)+1:floor((thisY-1)*1843.2)+size(rotatedtemplate,1),floor((thisX-1)*1843.2)+1:floor((thisX-1)*1843.2)+size(rotatedtemplate,1)),X);
    dapimosaic(floor((thisY-1)*1843.2)+1:floor((thisY-1)*1843.2)+size(rotatedtemplate,1),floor((thisX-1)*1843.2)+1:floor((thisX-1)*1843.2)+size(rotatedtemplate,1))=max(dapimosaic(floor((thisY-1)*1843.2)+1:floor((thisY-1)*1843.2)+size(rotatedtemplate,1),floor((thisX-1)*1843.2)+1:floor((thisX-1)*1843.2)+size(rotatedtemplate,1)),dapi);
...
save(fullfile(matlabtempfiles,'seedsregion0optiNoOverlapAGAINSmallRotation2.mat'),'seedsmosaic','-v7.3')
...
save(fullfile(matlabtempfiles,'Searea0SmallRotation2.mat'),'Se','-v7.3')
imwrite(reduceImage(Se),fullfile(matlabtempfiles,'reducedSearea0SmallRotation2.tif'));
clear Se
SD=reduceImage(reduceImage(dapimosaic));
save(fullfile(matlabtempfiles,'SDarea0SmallRotation2.mat'),'SD','-v7.3')
imwrite(reduceImage(SD),fullfile(matlabtempfiles,'reducedSDarea0SmallRotation2.tif'));
clear SD
