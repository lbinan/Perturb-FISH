...
globalmerfishspots=globalmerfishspots(globalmerfishspots(:,end-3)<151000,:);
...
ids=sub2ind([size(mymask,1),size(mymask,2)],globalmerfishspots(:,end-3)-98800+1,globalmerfishspots(:,end-2));
...
globalmerfishspots=globalmerfishspots(globalmerfishspots(:,end-3)<85600,:);
globalmerfishspots=globalmerfishspots(globalmerfishspots(:,end-2)<67200,:);
% spotsimage=zeros(size(mymask));
% for i=1:size(globalmerfishspots,1)
%     spotsimage(floor(globalmerfishspots(i,end-3)),floor(globalmerfishspots(i,end-2)))=globalmerfishspots(i,1);
% end
mymask=mymask>0;
%  ids=find(spotsimage)>0;
ids=sub2ind([size(mymask,1),size(mymask,2)],globalmerfishspots(:,end-3)-32000+1,globalmerfishspots(:,end-2));
globalmerfishspots(:,13)=ids;
cellstats=regionprops(mymask,'Area','PixelIdxList');
for i=1:size(cellstats,1)
    mypixels=cellstats(i).PixelIdxList;
