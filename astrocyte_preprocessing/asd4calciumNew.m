matlabtempfiles='/broad/clearylab/Users/Loic/ASdRevsZombiedish4/matlabtempfiles/'
%% region 0
calciumpath='/broad/clearylab/Users/Loic/astrocytesFeb2/dish4/20240202_105738_544'

matlabmerfish='/broad/clearylab/Users/Loic/ASDMerfishRevs4/matlabtempfiles'
transformpath='/broad/clearylab/Users/Loic/ASdRevsZombiedish4/outputs'
load(fullfile(matlabmerfish,'mymaskregion0V2.mat'))
mymask=bwlabel(mymask>0);
mymask=downsample(mymask',30);
mymask=downsample(mymask',30);
mymask=flipdim(mymask,1);
load(fullfile(transformpath,'tformMerfishstraighttocalciumregion0saple4.mat'))
mymask=imrotate(mymask,-90);
mymask = imwarp_same(mymask,tform);
imwrite(mymask,fullfile(matlabtempfiles,'preppedmaskR0.tif'))
% recoveredmerfish=recoveredmerfish(1:1480,1:1480);%calcium images are 1480x1480
% recoveredmerfish=recoveredmerfish(:,3100:5000);
% recoveredmerfish=[zeros(size(recoveredmerfish,1),193),recoveredmerfish];
% recoveredmerfish=recoveredmerfish(:,3100:5000);
% imwrite(recoveredmerfish,fullfile(matlabtempfiles,'mappedmerfish.tif'))

numberofcells=max(max(mymask));

%loadstack
numberofimages=249
calciumsignal=zeros(numberofcells,numberofimages);

% load(fullfile(matlabtempfiles,'tformMtoCfromalginedshrunkandroratedMregion0.mat'))
for calciumimage=1:numberofimages
    if calciumimage==1
        myimage=imrotate(imread(fullfile(calciumpath,'Time00000_ChannelSS_FITC1_Seq0000.tiff')),-90);
        mymask=mymask(1:size(myimage,1),1:size(myimage,2));
    else
        if calciumimage<10
            myimage=imrotate(imread(fullfile(calciumpath,strcat('Time0000',num2str(calciumimage),'_ChannelSS_FITC1_Seq000',num2str(calciumimage),'.tiff'))),-90);
        elseif calciumimage<100
            myimage=imrotate(imread(fullfile(calciumpath,strcat('Time000',num2str(calciumimage),'_ChannelSS_FITC1_Seq00',num2str(calciumimage),'.tiff'))),-90);
        elseif calciumimage<1000
            myimage=imrotate(imread(fullfile(calciumpath,strcat('Time00',num2str(calciumimage),'_ChannelSS_FITC1_Seq0',num2str(calciumimage),'.tiff'))),-90);
        end
    end
    for thiscell=1:numberofcells
        pixelIds=find(mymask==thiscell);
        if max(pixelIds)<size(myimage,1)*size(myimage,2)
            calciumsignal(thiscell,calciumimage)=mean(myimage(pixelIds));
        end
    end
end
disp('finished 1st table')
imwrite(mymask,fullfile(matlabtempfiles,'croppedToCalciumMasksample4.tif'))
imwrite(myimage/100,fullfile(matlabtempfiles,'mappedcalciumsample4.tif'))
writematrix(calciumsignal,fullfile(matlabtempfiles,'calciumsignalASD4region0savedmemory.csv'))