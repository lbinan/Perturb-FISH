...
%% region 0
calciumpath='/broad/clearylab/Users/Loic/astrocytesFeb2/dish3/20240202_105210_586well2'

matlabmerfish='/broad/clearylab/Users/Loic/asdrevsdish3merfish/202402171543_loicAsdRevsDish3_VMSC11302/matlabtempfiles'
transformpath='/broad/clearylab/Users/Loic/loicAsdRev3_zombie/outputs'
load(fullfile(matlabmerfish,'mymaskregion0V2.mat'))
mymask=bwlabel(mymask>0);
mymask=downsample(mymask',30);
mymask=downsample(mymask',30);
mymask=flipdim(mymask,1);
load(fullfile(transformpath,'tformMerfishstraighttocalciumregion0saple3.mat'))
...
for calciumimage=1:numberofimages
    if calciumimage==1
        myimage=imrotate(imread(fullfile(calciumpath,'Time00000_ChannelSS_FITC1_Seq0000.tiff')),90);
        mymask=mymask(1:size(myimage,1),1:size(myimage,2));
    else
        if calciumimage<10
            myimage=imrotate(imread(fullfile(calciumpath,strcat('Time0000',num2str(calciumimage),'_ChannelSS_FITC1_Seq000',num2str(calciumimage),'.tiff'))),90);
        elseif calciumimage<100
            myimage=imrotate(imread(fullfile(calciumpath,strcat('Time000',num2str(calciumimage),'_ChannelSS_FITC1_Seq00',num2str(calciumimage),'.tiff'))),90);
        elseif calciumimage<1000
            myimage=imrotate(imread(fullfile(calciumpath,strcat('Time00',num2str(calciumimage),'_ChannelSS_FITC1_Seq0',num2str(calciumimage),'.tiff'))),90);
        end
    end
...
writematrix(calciumsignal,fullfile(matlabtempfiles,'calciumsignalASD3region0savedmemory.csv'))
%% region 1
matlabtempfiles='/broad/clearylab/Users/Loic/loicAsdRev3_zombie/matlabtempfiles'
calciumpath='/broad/clearylab/Users/Loic/astrocytesFeb2/dish3/20240202_104903_338well1'
...
mymask=downsample(mymask',30);
mymask=flipdim(mymask,1);
load(fullfile(transformpath,'tformMerfishstraighttocalciumregion1saple3.mat'))
mymask=imrotate(mymask,-90);
mymask = imwarp_same(mymask,tform);
...
imwrite(myimage/100,fullfile(matlabtempfiles,'mappedcalciumR1well1.tif'))
writematrix(calciumsignal,fullfile(matlabtempfiles,'calciumsignalASD3region1savingmemory.csv'))